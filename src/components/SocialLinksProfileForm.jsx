...
import { FaXTwitter } from 'react-icons/fa6'; // Adjust imports as needed
        ...
        <div className="mb-4">
          {socialLinks.map((socialLink, index) => (
            <div key={index} className="relative flex gap-x-3 mb-4">
              <label htmlFor={socialLink.name} className="flex text-lg h-10 items-center">
                {iconMap[socialLink.name] || <FaLink />} {/* Default icon if not found */}
              </label>
              <input
                type="text"
                id={socialLink.name}
